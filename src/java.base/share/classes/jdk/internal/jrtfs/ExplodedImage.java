...
class ExplodedImage extends SystemImage {
     ...
     */
    Path underlyingModulesPath(String str) {
        if (str.startsWith(MODULES) && str.length() > MODULES.length()) {
            Path path = modulesDir.resolve(frontSlashToNativeSlash(str.substring(MODULES.length())));
